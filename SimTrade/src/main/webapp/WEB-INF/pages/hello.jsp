...
<form id="envselection" action="result" method="post" onsubmit="submitForm()">
 <table border="2">
  ...
  <c:forEach items="${simDetails}" var="entry" varStatus="count">
   ...
   <td>${entry.simId}</td>
    ...
    <c:when test="${entry.currentStatus=='passive'}">
        <td><input type="radio"  name="env" id="radioSelection" value="${entry.userName}_${entry.country}" disabled="disabled"></td>
        
    </c:when>    
    <c:otherwise>
         
        <td><input type="radio" name="env" id="radioSelection" value="${entry.userName}_${entry.country}" ></td>
  ...
  <tr><td>
   <br>
  <input type="submit" name="confirmButton" value="Confirm" style="height:50px; width:100px"></input></td></tr>
 </table>
